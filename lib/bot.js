function BotFactory(BaseBot) {
    return function Bot(token, url, port) {
        ...
        const helpText = () => {
            let share = `, then 'Share' the message`;
            return `To ask for money in amount of N, type /debt N${share}.\n`
                 + `To give money in amount of N, type /debt -N${share}.\n`
                 + `To view your debts, type /stats.\n`
                 + `To share this bot, type /share${share}.\n`
                 + `Inline mode — @${name} N (-N) or @${name} share.\n`;
        };
        ...
        const shareText = () =>
            `Hi. My name is @${name}. I can manage all your debts. See you?`;
