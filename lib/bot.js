...
const BotFactory = (BaseBot) => function Bot(token, admin) {
    ...
    var deleteUser = (name) => {
        ...
        let debt = debts.get(name);
        if (debt) debt.forEach((_, debtor) => debts.get(debtor).delete(name));

        debts.delete(name);

        let deal = deals.get(name);
        if (deal) deal.forEach((_, dealer) => deals.get(dealer).delete(name));
    ...
    bot.onText(/\/debt/, (msg, match) => {
        bot.tailMessage(msg.from.username,
            ...
            `If n > 0, asks @username for a debt in amount of n. Otherwise, asks @username to owe you in amount of -n`);
    });

    var getNumber = (str) => Number(str.split('.')[0] + str.split('.')[1]);
            ...
            bot.sendMessage(toId, 'ping').then(
                (ping) => {
                    ...
                    if (!!toDeals.get(from)) {
                        ...
                        var amount = getNumber(match[2]);
                        toDeals.set(from, amount);

                        bot
                        .sendMessage(toId, `${amount > 0 ? `@${from} owes you ${amount}` : `you owe ${amount} to @${from}`}. Is that right? (/decline | /accept)`)
                ...
                bot.sendMessage(toId, 'ping').then(
                    (ping) => {
                        ...
                        if (accept) {
                            ...
                            let val = fromDebts.set(to, fromDebts.get(to) + amount);
                        ...
                        .then(() => bot.tailMessage(to, debtMsg(amount < 0, from, accept), from));
                    },
                    () => bot.notifyDeletion(to, from)
                );
            }
    ...
    var getText = (num) => String(num).slice(0, -2) + '.' + String(num).slice(-2);

    var statsMsg = (name) => {
        let result = 'Debts:\n';

        if (!!debts.size && !!debts.get(name))
            debts.get(name).forEach((amount, debtor) => result += `\n@${debtor} ${getText(amount)}`);
        else
            result += '\nno debts, no owes.';

        result += '\n\nDeals:\n';

        if (!!deals.size && !!deals.get(name))
            deals.get(name).forEach((amount, dealer) => result += `\n@${dealer} ${getText(amount)}`);
        else
            result += '\nno deals.';
    ...
    bot.onText(/\/save (.+)/, (msg, match) => {
        if (match[1] == admin) {
            // save everything before shutdown and send file to admin
        }
    })
