...
Inline mode — @${name} N or @${name} share.`))
    ...
    bot.onText(/\/debt$/, msg =>
        ...
        .then(
            () => bot.sendMessage(msg.chat.id, `Send amount`))
        .catch(
            error => console.log(error)
        )
    );
        ...
        if (debtRegexp.test(query.query)) {
            ...
            answer = {
                ...
                input_message_content: {
                    message_text: userText(match[2], data.amount) || debtMsg(data.amount, data.name)
                },
                reply_markup: {
                    inline_keyboard: [
                        [
                            { text: '✔️', callback_data: offerData(data, true) },
