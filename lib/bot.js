...
class Bot {
    constructor(token, url, port) {
        ...
        bot.onText(/\/start/, msg => {
            bot.sendMessage(msg.chat.id, startText()).catch(logger);
        });
        ...
        bot.onText(/\/help/, msg => {
            bot.sendMessage(msg.chat.id, helpText()).catch(logger);
        });
        ...
        bot.onText(/\/share/, msg => {
            bot.sendMessage(msg.chat.id, shareText(), shareKeyboard).catch(logger);
        });
        ...
        function inlineShare(query) {
            ...
            bot.answerInlineQuery(query.id, [answer]).catch(logger);
        };
        ...
        bot.onText(/\/stats/, msg => {
            dataBase.getStats(msg.from.username)
                .then(table => {
                    return bot.sendMessage(msg.chat.id, statsText(table), { reply_markup: statsKeyboard });
                })
                .catch(logger);
        });
        ...
        function updateStats(query) {
            ...
            dataBase.getStats(query.from.username)
                ...
                .then(() => {
                    return bot.sendMessage(query.message.chat.id, statsText(stats), { reply_markup: statsKeyboard });
                })
                .catch(logger);
        };
        ...
        function statsText(table) {
            if (!table.length)
                return '';
            let debts = table.filter(debt => debt.amount > 0), owes = table.filter(debt => debt.amount < 0)
                .map(lineAbs);
            return ''
        ...
        bot.onText(debtRegexp, (msg, match) => {
            if (match[1].length < DIGITS_LIMIT)
                bot.sendMessage(msg.chat.id, debtText(match[2], Number(match[1]), msg.from.username), {
                    reply_markup: JSON.stringify({
                        inline_keyboard: [[{
                ...
                bot.sendMessage(msg.chat.id, `❌ Размер долга нереально большой ❌`).catch(logger);
        });
        ...
        function onOfferClick(query) {
            ...
            if (offer.to == offer.from) {
                if (offer.accept) {
                    bot.answerCallbackQuery(query.id, { text: `Нельзя должать себе` }).catch(logger);
                } else {
                    bot.editMessageText(`Отменено @${offer.from}`, { inline_message_id: query.inline_message_id }).catch(logger);
                }
            } else {
                bot.editMessageText(closedDealMsg(offer), { inline_message_id: query.inline_message_id })
                    .then(() => {
                        if (offer.accept)
        ...
        function debtArticle(offer, match) {
            return {
                type: 'article',
                id: articleID(),
        ...
        async function emptyInline(query) {
            try {
